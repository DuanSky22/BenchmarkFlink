 ...
 * <br/>
 * A triangle is a 3-cycle with vertices A, B, and C connected by edges.txt
 * (A, B), (A, C), and (B, C).
 * <br/>
 * The input graph must be a simple, undirected graph containing no duplicate
 * edges.txt or self-loops.
 * <br/>
...
extends GraphAlgorithmDelegatingDataSet<K, VV, EV, Tuple3<K, K, K>> {
	...
	private DataSet<Tuple3<K, K, K>> result;
    ...
    public DataSet<Tuple3<K,K,K>> addEdge(Edge<K, EV> edge){
        K source = edge.getSource();
        K target = edge.getTarget();

        result = graph.getEdges().flatMap(new FlatMapFunction<Edge<K,EV>, K>() {
            @Override
            public void flatMap(Edge<K, EV> value, Collector<K> out) throws Exception {
                if(value.getSource().equals(source) || value.getSource().equals(target))
                    out.collect(value.getTarget());
                else if(value.getTarget().equals(source) || value.getTarget().equals(target))
                    out.collect(value.getSource());
            }
        }).groupBy("*").reduceGroup(new GroupReduceFunction<K, K>() {
            @Override
            public void reduce(Iterable<K> values, Collector<K> out) throws Exception {
                int counter = 0; K value = null;
                Iterator<K> iter = values.iterator();
                while(iter.hasNext()){
                    counter++;
                    value = iter.next();
                }
                if(counter > 1) out.collect(value);
            }
        }).map(new MapFunction<K, Tuple3<K,K,K>>() {
            @Override
            public Tuple3<K, K, K> map(K value) throws Exception {
                return new Tuple3<K,K,K>(source,value,target);
            }
        }).union(result);
        return result;
    }
